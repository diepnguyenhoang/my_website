...
The website I am going to build is a **static web** which is built with much simple codes/language, allow simple navigation ... in comparison to dynamic web [Wiki](https://en.wikipedia.org/wiki/Static_web_page). In a static site, the HTML code is fixed and is not generated dynamically per HTTP Requests.

In my case, I will use **static web generator**, namely **MKDOCS**, to help me simplify the process of building a website. MKDOCS is considered as a fast, simple and downright gorgeous static site generator that's geared towards building project documentation. 
...
7. Update the remote repo with following commands
	...
	*Git add .* will add any changes I made above in the working directory to the remote repo. It will tell Git to include updates to a particular file in the next commit.
